...
*   **Functionality**: Generates dynamic sentence construction exercises. The user inputs a target verb, topic, or grammar rule along with their CEFR level (A1-C2). The local LLM generates an English prompt and provides the corresponding Spanish translation as jumbled, interactive tiles (including optional distractors).
*   **User Experience**: Features a drag-and-drop workspace (or staging area) where users can assemble and reorder the tiles to form the complete sentence. Instead of restrictive, frustrating word-by-word error checking, users submit their completed sentence for evaluation.
